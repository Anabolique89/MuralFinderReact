...
const BlogService = {
  getAllBlogPosts: async () => {
    try {
      const response = await axios.get(`${BASE_URL}${blogEndpoints.getAllBlogPosts}`); // Adding BASE_URL to the endpoint
  ...
  getCommentsForBlogPost: async (postId) => {
    try {
      const response = await axios.get(`${BASE_URL}${blogEndpoints.getCommentsForBlogPost(postId)}`);
      console.log(response.data.data)
      return response.data.data;
    } catch (error) {
